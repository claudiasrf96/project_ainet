...
<v-data-table :headers="headers" :items="invoice" item-key="id" :pagination.sync="pagination" :total-items="totalInvoices" :rows-per-page-items="pagination.rowsPerPageItems" >
  <template  slot="items" slot-scope="props" >
        <tr @click="props.expanded = !props.expanded">
            ...
            <td class="text-xs-left">{{ props.item.meals.table_number }}</td>
  ...
  export default {
    ...
    methods: {
        getPage(page){ 
            ...
            axios.get('api/invoice/invoiceInfo/manager?page=' + (this.pagination.page ) + '&page_size=' + this.pagination.rowsPerPage).then(response=>{  
